...
export default function Cards(props) {
    ...
    var backgroundImageColor = {
        backgroundImage: `${props.color}, url(${props.img})`,
    };
    ...
    return (
        ...
        <Link to={props.url}>
            <div className="aspect-square bg-cover mt-4 transition ease-in-out delay-50" style={isHovered ? backgroundImageColor : backgroundImage} onMouseOver={()=>{hoverCard(true)}} onMouseLeave={()=>{hoverCard(false)}}>
                <div className="w-full h-full flex flex-col px-4 py-6 text-end items-end justify-end hover:-translate-y-3 hover:md:-translate-y-10 transition ease-in-out delay-50">
                    ...
                    <p className={`text-white text-clampTextSm ${isHovered ? hoverStyle : normalStyle}`}>{props.description}</p>
                    <br />
                    <p className={`text-white text-clampTextSm md:text-clampTextBig ${isHovered ? hoverStyle : normalStyle}`} dangerouslySetInnerHTML={{__html: props.investment}}></p>
